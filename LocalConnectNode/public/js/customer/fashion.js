...
class FashionPage {
    ...
    loadVendors() {
        ...
        const vendors = vendorsFromServer.map(v => ({
            id: v.id,
            name: v.name,
            category: categoryMap[v.subcategory] || 'ladies',
            rating: v.rating || 0,
    ...
    renderVendors() {
        const ladiesList = document.getElementById('ladies-list');
        const gentsList = document.getElementById('gents-list');
        const boyList = document.getElementById('boy-list');
        const girlList = document.getElementById('girl-list');

        if (ladiesList) ladiesList.innerHTML = '';
        if (gentsList) gentsList.innerHTML = '';
        if (boyList) boyList.innerHTML = '';
        if (girlList) girlList.innerHTML = '';

        const ladiesVendors = this.filteredVendors.filter(v => v.category === 'ladies');
        const gentsVendors = this.filteredVendors.filter(v => v.category === 'gents');
        const boyVendors = this.filteredVendors.filter(v => v.category === 'boy');
        const girlVendors = this.filteredVendors.filter(v => v.category === 'girl');
        ...
        ladiesVendors.sort(sortByDistance);
        gentsVendors.sort(sortByDistance);
        boyVendors.sort(sortByDistance);
        girlVendors.sort(sortByDistance);

        ladiesVendors.forEach(vendor => {
            if (ladiesList) ladiesList.appendChild(this.createVendorCard(vendor));
        });
        gentsVendors.forEach(vendor => {
            if (gentsList) gentsList.appendChild(this.createVendorCard(vendor));
        });
        boyVendors.forEach(vendor => {
            if (boyList) boyList.appendChild(this.createVendorCard(vendor));
        });
        girlVendors.forEach(vendor => {
            if (girlList) girlList.appendChild(this.createVendorCard(vendor));
        });
            ...
            if (response.ok) {
                ...
                const mergedResults = apiResults
                    ...
                    .map(apiVendor => {
                        ...
                        return {
                            ...apiVendor,
                            category: categoryMap[apiVendor.subcategory] || 'ladies',
