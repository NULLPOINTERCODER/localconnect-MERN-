...
class PharmacyPage {
    ...
    loadVendors() {
        ...
        const vendors = vendorsFromServer.map(v => ({
            id: v.id,
            name: v.name,
            category: categoryMap[v.subcategory] || 'ayurvedic',
            rating: v.rating || 0,
    ...
    renderVendors() {
        const ayurvedicList = document.getElementById('ayurvedic-list');
        const chemistList = document.getElementById('chemist-list');
        const babyList = document.getElementById('baby-list');

        if (ayurvedicList) ayurvedicList.innerHTML = '';
        if (chemistList) chemistList.innerHTML = '';
        if (babyList) babyList.innerHTML = '';

        const ayurvedicVendors = this.filteredVendors.filter(v => v.category === 'ayurvedic');
        const chemistVendors = this.filteredVendors.filter(v => v.category === 'chemist');
        const babyVendors = this.filteredVendors.filter(v => v.category === 'baby');
        ...
        ayurvedicVendors.sort(sortByDistance);
        chemistVendors.sort(sortByDistance);
        babyVendors.sort(sortByDistance);

        ayurvedicVendors.forEach(vendor => {
            if (ayurvedicList) ayurvedicList.appendChild(this.createVendorCard(vendor));
        });
        chemistVendors.forEach(vendor => {
            if (chemistList) chemistList.appendChild(this.createVendorCard(vendor));
        });
        babyVendors.forEach(vendor => {
            if (babyList) babyList.appendChild(this.createVendorCard(vendor));
        });
        ...
        try {
            ...
            if (response.ok) {
                ...
                const mergedResults = apiResults
                    .filter(v => v.category === 'Pharmacy')
                    .map(apiVendor => {
                        const localVendor = this.vendors.find(v => v.id === apiVendor.id);
                        return {
                            ...apiVendor,
                            category: categoryMap[apiVendor.subcategory] || 'ayurvedic',
