...
class BooksPage {
    ...
    loadVendors() {
        ...
        const vendors = vendorsFromServer.map(v => ({
            id: v.id,
            name: v.name,
            category: categoryMap[v.subcategory] || 'academic',
            rating: v.rating || 0,
    ...
    renderVendors() {
        ...
        if (academicList) academicList.innerHTML = '';
        if (novelsList) novelsList.innerHTML = '';
        if (stationeryList) stationeryList.innerHTML = '';

        const academicVendors = this.filteredVendors.filter(v => v.category === 'academic');
        const novelsVendors = this.filteredVendors.filter(v => v.category === 'novels');
        const stationeryVendors = this.filteredVendors.filter(v => v.category === 'stationery');
        ...
        academicVendors.sort(sortByDistance);
        novelsVendors.sort(sortByDistance);
        stationeryVendors.sort(sortByDistance);

        academicVendors.forEach(vendor => {
            if (academicList) academicList.appendChild(this.createVendorCard(vendor));
        });
        novelsVendors.forEach(vendor => {
            if (novelsList) novelsList.appendChild(this.createVendorCard(vendor));
        });
        stationeryVendors.forEach(vendor => {
            if (stationeryList) stationeryList.appendChild(this.createVendorCard(vendor));
        });
    ...
    createVendorCard(vendor) {
        ...
        if (vendor.matching_dishes && vendor.matching_dishes.length > 0) {
            ...
            matchingDishesHtml = `<div class="matching-dishes"><i class="fas fa-book"></i> ${dishList}${moreText}</div>`;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                const mergedResults = apiResults
                    .filter(v => v.category === 'Books')
                    .map(apiVendor => {
                        const localVendor = this.vendors.find(v => v.id === apiVendor.id);
                        return {
                            ...apiVendor,
                            category: categoryMap[apiVendor.subcategory] || 'academic',
