...
class GroceryPage {
    ...
    loadVendors() {
        ...
        const vendors = vendorsFromServer.map(v => ({
            id: v.id,
            name: v.name,
            category: categoryMap[v.subcategory] || 'vegetables',
            rating: v.rating || 0,
    ...
    renderVendors() {
        ...
        const dairy_eggsList = document.getElementById('dairy-eggs-list');
        const general_storeList = document.getElementById('general-store-list');
        const fruitsList = document.getElementById('fruits-list');

        if (vegetablesList) vegetablesList.innerHTML = '';
        if (dairy_eggsList) dairy_eggsList.innerHTML = '';
        if (general_storeList) general_storeList.innerHTML = '';
        if (fruitsList) fruitsList.innerHTML = '';

        const vegetablesVendors = this.filteredVendors.filter(v => v.category === 'vegetables');
        const dairy_eggsVendors = this.filteredVendors.filter(v => v.category === 'dairy-eggs');
        const general_storeVendors = this.filteredVendors.filter(v => v.category === 'general-store');
        const fruitsVendors = this.filteredVendors.filter(v => v.category === 'fruits');
        ...
        vegetablesVendors.sort(sortByDistance);
        dairy_eggsVendors.sort(sortByDistance);
        general_storeVendors.sort(sortByDistance);
        fruitsVendors.sort(sortByDistance);

        vegetablesVendors.forEach(vendor => {
            if (vegetablesList) vegetablesList.appendChild(this.createVendorCard(vendor));
        });
        dairy_eggsVendors.forEach(vendor => {
            if (dairy_eggsList) dairy_eggsList.appendChild(this.createVendorCard(vendor));
        });
        general_storeVendors.forEach(vendor => {
            if (general_storeList) general_storeList.appendChild(this.createVendorCard(vendor));
        });
        fruitsVendors.forEach(vendor => {
            if (fruitsList) fruitsList.appendChild(this.createVendorCard(vendor));
        });
            ...
            if (response.ok) {
                ...
                const mergedResults = apiResults
                    ...
                    .map(apiVendor => {
                        ...
                        return {
                            ...apiVendor,
                            category: categoryMap[apiVendor.subcategory] || 'vegetables',
